...
import eu.socialdancing.sdscan.droid.databinding.ActivityCheckinlistSelectBinding
...
class CheckInListSelectActivity : MorphingDialogActivity() {
    ...
    private fun getAllLists(): List<CheckInList> {
        ...
        var lists = (application as sdscan).data.select(CheckInList::class.java)
    ...
    private fun refresh(forceSync: Boolean = false) {
        ...
        if (forceSync || listOfLists.isEmpty() || subeventChanged) {
            (application as sdscan).syncLock.withLock {
                syncSync()
            }
        } else {
